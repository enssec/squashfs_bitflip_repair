...
static int gzip_uncompress(void *d, void *s, int size, int outsize, int *error)
{
	...
	/*
	 * HOOK START
	 * We try to handle errors, and fix broken chunks.
	 *
	 *  -A: compute the hash on the chunk (unique identifier)
	 *  -B: If not done yet, store the chunk in the data store 
     *      (`SDIR/<hash>.in`), with the suffix good if not broken
	 *  -C: If broken and the repaired file exists (`SDIR/<hash>.out`), 
     *      load it and decompress it instead of the broken chunk
	 *      Note: repair should be done by another program
	 */
    /* Step A: compute hash of chunk */
    unsigned char hash[SHA256_DIGEST_LENGTH];
    SHA256(s, size, hash);
    char hashstr[(((4*SHA256_DIGEST_LENGTH/3)+3)/4)*4+1];
    EVP_EncodeBlock((unsigned char *) hashstr, hash, SHA256_DIGEST_LENGTH);
    for(size_t i=0; i<sizeof(hashstr); i++) {
        char c = hashstr[i];
        if (c == '/') {
            c = '@';
        } else if (c == '+') {
    ...
    /* Step B: Create the 'in' file if absent */
    char infn[0x100];
    if(res != Z_OK)
        sprintf(infn, SDIR"%s.in", hashstr);
    else
        sprintf(infn, SDIR"%s.good", hashstr);
    
    if(access(infn, F_OK) == -1) {
        FILE *f = fopen(infn, "wb");
        if(f == NULL) {
            perror("fopen failed\n");
            exit(1);
        }
        size_t count_written = fwrite(s, size, 1, f);
        if (count_written != 1) {
    ...
    /* Step C: Load repaired chunk if it exists, and decompress it */
    if(res != Z_OK)
    {
        char outfn[0x100];
        sprintf(outfn, SDIR"%s.out", hashstr);
        FILE *f = fopen(outfn, "rb");
        if(f != NULL) {
            ERROR("Using repaired chunk '%s'.\n", hashstr);
            bytes = fread(d, 1, outsize, f);
            if (feof(stdin) != 0) {
                printf("fread fail\n");
                exit(1);
            }
            ...
            ERROR("Block '%s' has not been repaired yet.\n", hashstr);
        }
    }
	/*
	 * HOOK END
